...
void IMUTask::run()
{
    ...
    log_info("Initializing SPI1 with RX: %d, CS: %d, SCK: %d, TX: %d\n", SPI1_RX, SPI1_CS, SPI1_SCK, SPI1_TX);
    // IMU setup
    SPI1.setRX(SPI1_RX);
    SPI1.setCS(SPI1_CS);
    ...
    SPI1.setTX(SPI1_TX);
    SPI1.begin();
    log_info("SPI1 initialized.\n");

    if (asm330lhh.begin() == ASM330LHH_OK)
    {
        log_info("ASM330LHH initialized successfully\n");
    }
    else
    {
        log_error("Failed to initialize ASM330LHH\n");
        while (1)
    ...
    log_info("ASM330LHH enabled for accelerometer and gyroscope.\n");
